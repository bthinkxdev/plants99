...
class ProductDeliveryStateForm(forms.Form):
    ...
    Pricing is centralized on DeliveryState.delivery_charge (set from the
    Delivery Charges admin page) — this form only controls serviceability.
    """
    ...
    states = forms.ModelMultipleChoiceField(
        ...
        help_text=(
            ...
            "Delivery pricing is managed centrally from Admin → Delivery Charges."
        ),
    )
    ...
    def save(self):
        """Atomically replace the product's deliverable-state list."""
        if not self.product:
            return
...
class StateDeliveryChargeForm(forms.Form):
    """
    One delivery-charge input per active state — the single place admins set
    the fixed, centralized charge applied to every product shipping there.

    Fields are added dynamically as charge_<state_id>; blank clears the
    state back to "not configured" (flat-rate fallback applies).
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._grouped_states = get_states_by_region()
        for _region, states in self._grouped_states.items():
            for state in states:
    ...
    def save(self):
        ...
        for field_name, value in self.cleaned_data.items():
            state_id = int(field_name.replace("charge_", ""))
            charges[state_id] = value
        set_state_delivery_charges(charges)

    def get_states_by_region(self):
        """Returns (region_label, [(state, field), ...]) tuples for template rendering."""
        return [
            (REGION_LABELS.get(region, region), [
                (state, self[f"charge_{state.pk}"]) for state in states
            ])
            for region, states in self._grouped_states.items()
        ]
